(function ($) {
	...
	jQuery(function ($) {
		function updateNavigation() {
			if ($(window).width() > 990) {
				$(".navigation_mobile_main")
					.addClass("navigation")
					.removeClass("navigation_mobile");

				$(".navigation").find(".navigation_mobile_click").remove();
				$(".navigation ul").show(); // asegura que el menú no quede oculto
			} else {
				$(".navigation")
					.addClass("navigation_mobile navigation_mobile_main")
		...
		// Delegación del click en el botón mobile
		...
		// Hover en submenús
		$(document).on("mouseenter", ".navigation_mobile ul li", function () {
		...
		}).on("mouseleave", ".navigation_mobile ul li", function () {
			var sub_menu = $(this).find("ul:first");
	...
	jQuery(document).ready(function () {
		if (jQuery(".tp-banner").length) {
			...
			jQuery('.tp-banner').bind('revolution.slide.onchange', function (e, data) {
				var $counter = jQuery(".counter-slideshow h4");
				var layerStart = parseInt($counter.closest('.tp-caption').data('start')) || 0;

				// espera hasta que la capa haya aparecido
				setTimeout(function () {
					startCounter();
				}, layerStart + 200);
			});
	...
	function startCounter() {
		jQuery(".counter-slideshow h4").each(function () {
	...
	jQuery('.tp-banner').on('click', '.tp-leftarrow, .tp-rightarrow', function () {
